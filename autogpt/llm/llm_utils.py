...
def create_embedding(
    ...
    **kwargs,
    ...
    chunk_embeddings_weighted_sum = [0] * len(chunk_embeddings[0])
    for i in range(len(chunk_embeddings)):
        for j in range(len(chunk_embeddings[0])):
            chunk_embeddings_weighted_sum[j] += chunk_embeddings[i][j] * chunk_lengths[i]
    chunk_embeddings = [chunk_embeddings_weighted_sum[i] / total_length for i in range(len(chunk_embeddings[0]))]
    # normalize the length to one
    norm = math.sqrt(sum([x**2 for x in chunk_embeddings]))
    chunk_embeddings = [x/norm for x in chunk_embeddings]
